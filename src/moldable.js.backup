...
document.onreadystatechange = () => {
  ...
  if (edit) {
    ...
    const mutableUrl = new URL(location);
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.shiftKey && e.key === ":") {
    ...
    modal.querySelector("form").onsubmit = async (e) => {
      e.preventDefault();
      const input = e.target?.closest("dialog").querySelector("input").value.trim();
        ...
        const messages = [
          {
            role: "system",
            content:
              `Edit the following React program based on user's goal or instruction.
...
<script type="text/jsx">Your implementation here...<` +
              `/script>
  `.trim(),
          },
          {
